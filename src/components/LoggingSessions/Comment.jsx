...
const Comment = ({comment, commentEditOnPressHandler}) => {
  const renderCommentLines = text => {
    return (text || '').split('\n').map((line, index) => (
      <Text key={index.toString()} style={{fontSize: 14, color: '#000'}}>
        {line}
      </Text>
  ...
  return (
    <View
      ...
      <TouchableOpacity onPress={commentEditOnPressHandler}>
        <View
          style={{
            padding: 0,
            marginTop: 6,
          ...
          <View style={{flex: 1}}>{renderCommentLines(comment)}</View>
