...
export default function (state = initialState, action) {
  ...
  switch(action.type) {
    ...
    case 'DEVICE_CONNECTING':
      return  { ...state, connectionStateChanging: true, device: action.meta, status: 'connecting' }
    ...
    case 'DEVICE_CONNECTED':
      ...
      return  { ...state, connectionStateChanging: false, device: action.meta, status: 'connected', bondedDevicesFormatted: newBondedDevicesFormatted }

    case 'DEVICE_DISCONNECTING':
      return  { ...state, connectionStateChanging: true, device: action.meta, status: 'disconnecting' }
    ...
    case 'DEVICE_CLEAR_CONNECTED_DEVICE':
      ...
      return  { ...state, connectionStateChanging: false, device: null, status: 'disconnected',
        bondedDevicesFormatted: newBondedDevicesFormatted, wiping: false, sensorDataReceived: false,
        sensorError: false }
    ...
    case 'DEVICE_SET_BONDED_DEVICES':
      filteredFormattedBondedDevices = filterAndSortDevices(action.payload.bondedDevices,state.deviceIdNameHash,state)
      return  { ...state, bondedDevicesRaw: action.payload.bondedDevices, devicesInRange: filteredFormattedBondedDevices, bondedDevicesFormatted: filteredFormattedBondedDevices }
    ...
    case 'DEVICE_ADD_BONDED_DEVICE':
      ...
      deviceIdNameHash[action.payload.newBondedDevice.address] = action.payload.newBondedDevice.name
      filteredFormattedBondedDevices = filterAndSortDevices(newBondedDevicesRaw,deviceIdNameHash,state)
      return  { ...state, bondedDevicesRaw: newBondedDevicesRaw, bondedDevicesFormatted: filteredFormattedBondedDevices, deviceIdNameHash }

    case 'DEVICE_SET_DISCOVERED_DEVICES':
      const unpairedDevices = []
      const currentTimestamp = new Date().getTime()
      const devicesInRange = [ ...action.payload.discoveredDevices ]
      console.log("XXX action.payload.discoveredDevices",action.payload.discoveredDevices)
      action.payload.discoveredDevices
        .filter(({name, address}) => {
          const bondedDevice = state.bondedDevicesRaw.find((bondedDevice) => (bondedDevice.address === address))
          return (!bondedDevice &&
      ...
      state.bondedDevicesFormatted.forEach((bondedDevice) => {
        deviceInRange = devicesInRange.find((deviceInRange) => (deviceInRange.address === bondedDevice.address))
        newBondedDevice = { ...bondedDevice }
        if (deviceInRange) {
          newBondedDevice['inRange'] = true
          newBondedDevice['inRangeSince'] = currentTimestamp
        } else if (newBondedDevice['inRangeSince'] && ((currentTimestamp < newBondedDevice['inRangeSince']) < 120000)) {
          newBondedDevice['inRange'] = true
        } else {
      ...
      state.unpairedDevices.forEach((oldUnpairedDevice) => {
        const newDevice = unpairedDevices.find(({address}) => (oldUnpairedDevice.address === address) )
        if (!newDevice) {
