...
class LocationNotFound extends Component {
  ...
  renderBody = (props,state) => {
    ...
    if (!props.locationEnabled || (!props.lat && !props.lng)) {
      ...
      const searchingForLocation = (!locationDisabled && !props.lat && !props.lng)

      let message = ''
      if (locationDisabled) {
        message = "Location is disabled in your phone's settings. Please open Settings, go to Location and set it to On."
      ...
      return <View style={{ borderColor: '#CCC', borderWidth: 2, borderRadius: 10 }} >
        ...
        >
          {searchingForLocation && <ActivityIndicator size="small" color={lightColors.primary} style={{ marginBottom: 10 }} />}
          <Text>{message}</Text>
        </View>
      </View>
